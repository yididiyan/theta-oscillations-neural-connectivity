...
class LassoParentSelection():
    ...
    def __init__(self, config, dataset, 
        ...
        self.all_areas = sorted(self.all_areas)
    ...
    def generate_area_combinations(self):
        all_combos = [c for i in range(1, len(self.all_areas)) for c in list(itertools.combinations(self.all_areas, i)) ]
        return all_combos
    ...
    def prepare_data(self, Yunit, recording_id, standardize=False):
        ...
        def prepare_data_(stimulus_id, Xunits):
            ...
            '''
            filtered_spikes_path = self.filtered_spikes_path.replace(f'output_stimuli_{str(self.stimulus_id)}' , f'output_stimuli_{str(stimulus_id)}') ## Switch to another stimulus 
    ...
    def __call__(self):
        ...
        for Yunit_id in units:
            ...
            if DI_Y_X[self.stimulus_id] > 0:
                ...
                # Latest update 
                # the key for area_models should be a set of areas... not a single area 
                # generate the combination 
                all_combos = self.generate_area_combinations()

                area_HY_X = {combo: {} for combo in all_combos}
                area_HY_X_all = {combo: None for combo in all_combos} # with all data combined 

                # import ipdb; ipdb.set_trace()

                for area_combo in all_combos:
                    # get units information - idxs and unit ids  
                    unit_ids = []
                    unit_indices = []
                    for a in area_combo: 
                        unit_indices.extend(subset_indices[a]['all']) 
                        unit_ids.extend(area_units[a])

                    
                    if not unit_indices:
                        print('No units from the union of areas were found ')
                        continue

                    results['area_models'][area_combo] = {}
                    # print(area_units)
                    # include features 0, 1,  Y's own features  
                    indices_ = [0, 1] + unit_indices
                    area_models[area_combo], area_best_set = self.fit_cv_model(Yunit_id, unit_ids, Past_train.T[indices_,:].T, Yvec_train, Cs=self.Cs)


                    for stimulus_id in Past_test.keys():
                        area_HY_X[area_combo][stimulus_id] = LassoParentSelection.H(area_models[area_combo].predict_proba(Past_test[stimulus_id].T[indices_, :].T), Yvec_test[stimulus_id])
                    
                    area_HY_X_all[area_combo] = LassoParentSelection.H(area_models[area_combo].predict_proba(Past_test_all.T[indices_, :].T), Yvec_test_all)
                    
                    print(f'from area {area_combo} -> {Yunit_area},  HY_X  {area_HY_X[area_combo][self.stimulus_id]} DI {HY[self.stimulus_id] - area_HY_X[area_combo][self.stimulus_id]}')
                    # area_models

                    results['area_models'][area_combo]['HY_X'] = area_HY_X[area_combo]
                    results['area_models'][area_combo]['HY_X_all'] = area_HY_X_all[area_combo]
                    results['area_models'][area_combo]['C'] = area_models[area_combo].C_
                    results['area_models'][area_combo]['selected_parents'] = area_best_set

                    # sanity - check  would be to see if area_best_set is from areas in area combo
